...
function initHost(root) {
	...
	console.log("[vhost manager] Initializing '%s' ...", hostname);
	...
	fs.readFile(hostFilePath, function(err, data) {
		...
		if(err || data.length == 0) {
			if(vhostManagerSettings.developerMode) {
				console.warn("[vhost manager] Could not read '%s' for '%s'. Using default.", vhostManagerSettings.hostFilename, hostname);
			}
		...
		try {
			...
			console.error("[vhost manager] Could not load '%s' for '%s'. Host unresponsive.", hostSettings.main, hostname);
			if(err.errno != 34) {
				console.error(err);
			}
		}
		
		//fs.readFile(root + "/" + hostSettings.main, function(err, data) {
		//	if(err) { return; }
		//	
		//	hostModule._compile(data, hostSettings.main);
		//});
